...
class CVEMatcher:
    ...
    def search(self, keyword):
        ...
        try:
            ...
            if self.cfg["cve_api_key"]:
                headers["apiKey"] = self.cfg["cve_api_key"]
            params = {"keywordSearch": keyword, "resultsPerPage": 10}
            r = self.s.get(self.cfg["cve_api"], params=params, headers=headers, timeout=15)
            ...
            for v in data.get("vulnerabilities", [])[:10]:
                ...
                cves.append({"id": cve_id, "cvss": cvss, "desc": desc, "refs": refs})
                good(f"CVE: {cve_id} (CVSS {cvss})")
            return cves
        except Exception as e:
    ...
    def match_services(self, port_scan):
        section("CVE Matching (NVD NIST)")
        all_cves = []
        seen = set()
        for p in port_scan.get("ports", []):
            banner = p.get("banner","")
            version_match = re.search(r"([\w.\-]+)[/\s]([\d]+\.[\d.]+)", banner)
            keyword = version_match.group(0) if version_match else p["service"]
            if not keyword or keyword in ("unknown",""):
                continue
            if keyword in seen: continue
            seen.add(keyword)
            info(f"Searching CVEs for: {keyword}")
            cves = self.search(keyword)
            if cves:
                all_cves.append({"service": keyword, "port": p["port"], "cves": cves})
...
class ExploitSuggester:
    @staticmethod
    def suggest(port_scan, cves):
        section("Exploit Suggestions")
        exploits = []
        for p in port_scan.get("ports", []):
            svc = p["service"]
            if svc in EXPLOIT_HINTS:
                info(f"Port {p['port']} ({svc}):")
                for h in EXPLOIT_HINTS[svc]:
                    exploits.append({"port": p["port"], "service": svc, "exploit": h})
                    console.print(f"    [yellow]•[/] {h}") if RICH_AVAILABLE else print(f"    • {h}")
        for c in cves:
            for entry in c.get("cves", []):
                exploits.append({"cve": entry["id"], "cvss": entry["cvss"],
                                 "exploit": f"Search msfconsole: search {entry['id']}",
                                 "refs": entry.get("refs",[])})
        return exploits
...
PLUGINS = {
    ...
    "cve":            ("CVE matching via NVD NIST API", CVEMatcher),
    ...
    "exploits":       ("Exploit suggestions", ExploitSuggester),
}
...
def _sev_badge(sev):
    ...
    return f'<span class="badge" style="background:{c}22;color:{c};border:1px solid {c}55">{sev.upper()}</span>'
...
def _count_issues(data):
    ...
    def walk(obj):
        if isinstance(obj, dict):
            ...
            if sev and sev.lower() in counts:
                counts[sev.lower()] += 1
            for v in obj.values():
                walk(v)
...
def _render_value(v):
    """Render a JSON-like value as a readable HTML fragment (table for list-of-dicts, else pretty JSON)."""
    if isinstance(v, list) and v and all(isinstance(i, dict) for i in v):
        ...
        for item in v:
            ...
            for k in keys:
                cell = item.get(k, "")
                if k == "severity":
                    cell = _sev_badge(cell)
                elif isinstance(cell, (list, dict)):
                    cell = f"<code>{json.dumps(cell, default=str)}</code>"
    ...
    if isinstance(v, dict):
        ...
        for k, val in v.items():
            if k == "issues" and isinstance(val, list) and val:
                items = "".join(
                    f'<div class="issue-row">{_sev_badge(i.get("severity"))} <span>{i.get("issue","")}</span></div>'
                    for i in val)
                rows.append(f"<tr><td class='k'>{k}</td><td>{items}</td></tr>")
            elif isinstance(val, (list, dict)):
                rows.append(f"<tr><td class='k'>{k}</td><td><pre>{json.dumps(val, indent=2, default=str)}</pre></td></tr>")
            else:
                rows.append(f"<tr><td class='k'>{k}</td><td>{val}</td></tr>")
...
def _render_html_report(data):
    ...
    stat_cards = f"""
      ...
      <div class="stat"><div class="stat-num">{ssl_grade}</div><div class="stat-label">SSL/TLS Grade</div></div>
...
<style>
  ...
  * {{ box-sizing: border-box; }}
  ...
  table.kv td.k {{ color: var(--muted); width: 220px; font-weight:600; }}
  .table-wrap table {{ border: 1px solid var(--border); border-radius:10px; overflow:hidden; }}
  .table-wrap th {{ text-align:left; background: var(--panel-2); color: var(--muted); padding:10px; font-size:12px; text-transform:uppercase; letter-spacing:.4px; }}
  .table-wrap td {{ padding:10px; border-top:1px solid var(--border); }}
  .table-wrap tr:hover td {{ background: rgba(34,211,238,0.04); }}

  pre {{ background:#0b1220; padding:10px 12px; border-radius:8px; overflow-x:auto; font-size:12px; color:#c7d2fe; margin:0; }}
...
def run_scan(target, modules, cfg, ports=None):
    ...
    if "portscan" in modules or "all" in modules:
        ...
        if "exploits" in modules or "all" in modules:
            result.exploits = ExploitSuggester.suggest(result.port_scan, result.cves)
